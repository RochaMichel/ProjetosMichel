...
User Function gpdvfat(cProcesso)
	...
	While (cZR23Alias)->(!EOF())
		...
		While (cZR23Alias)->(!EOF()) .AND. (cZR23Alias)->(ZR2_FILIAL+ZR2_NPRO) == cChaveNpro
			...
			(cZR23Alias)->ZR2_NPRO	,;//6 -- Cod. Processo
			(cZR23Alias)->ZR2_CLI	,;//7 -- Cod. Cliente
			(cZR23Alias)->ZR2_CLOJA	,;//8 -- Cod. Loja do Cliente 
			(cZR23Alias)->ZR3_LOCAL	,;//9 -- Armaz�m
			(cZR23Alias)->ZR2_USER  ,;//10-- Cod. Usu�rio 
			(cZR23Alias)->ZR3_UM  	,;//11-- Uni. Medida
			(cZR23Alias)->ZR2_OPER})//13-- Operacao
			(cZR23Alias)->(DbSkip())
			//(cZR23Alias)->ZR2_CONDPS,;//12-- Cond. pag. saida
		End
		// Capturo os valores constantes 
		cTes		:= aLlTrim(aDados[1,5])
		cNpro		:= aLlTrim(aDados[1,6])
		cCliente 	:= aLlTrim(aDados[1,7])
		cLoja 		:= aLlTrim(aDados[1,8])
...
Static Function GeraPdVd(cCodCli, cLojCli, cNpro, aDados)
	...
	**/*TABELA SC5 CABE�ALHO DO PEDIDO DE VENDA*/**
	...
	aAdd(aCabPv,{"C5_CONDPAG"	, aDados[1,12]												,Nil}) //11
	...
	aAdd(aCabPv,{"C5_XMSGI"   	, cMemo														,Nil}) //18
	...
	For nX:= 1 To Len(aDados)
		...
		**/*TABELA SC6 ITENS DO PEDIDO DE VENDA*/**
		...
		aAdd(aItePv,{"C6_UM"      	,aDados[nX,11]												,Nil}) //08
		...
		aAdd(aItePv,{"C6_OPER"    	,aDados[nX,13]												,Nil}) //09
		//aAdd(aItePv,{"C6_TES"   	,AllTrim(aDados[nX,5])										,Nil}) //10
		aAdd(aItePv,{"C6_LOCAL"		,aDados[nX,9]												,Nil}) //11
