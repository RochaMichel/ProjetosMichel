...
* Função : bol011
*----------------------------------------------------------------------------
* Objetivo :  Imprime o boleto tanto do_ Banco do_ Brasil como do_ Bradesco
*             no formato A4, gera até 3 boletos por folha do_ mesmo título.
*----------------------------------------------------------------------------
* Alterações : Ajustes para rotina ser chamada automaticamente após a emissão
*              da DANFE
*         Em : 02/08/2011
*----------------------------------------------------------------------------

// COLOCAR TRAVA PARA NAO PERMITIR EMISSAO DE BOLETO SEM A TRANSMISSAO DA NF

User Function BOL011()
...
//cFilter		+= "E1_PORTADO <> '  ' .AND. E1_PORTADO = '001' .AND. "

//If !empty(MV_PAR01)
//	cFilter		:= "E1_CARGA = '" + ALLTRIM(MV_PAR01) + "' .and. "
//EndIf

//If MV_PAR03 == 2 //Exibir só os boletos ja impressos
//	cFilter		+= "E1_NUMBCO <> '' .and. "
//ElseIf MV_PAR03 == 3 //Exibir só os boletos nao impressos
//	cFilter		+= "E1_NUMBCO = '  ' .and. "
//EndIf

cFilter		:= "E1_NUM    >= '" + ALLTRIM(MV_PAR01) + "' .And. "
cFilter		+= "E1_NUM    <= '" + ALLTRIM(MV_PAR02) + "' .And. "
cFilter		+= "E1_FILIAL == '" + xFilial("SE1")    + "' .And. E1_SALDO > 0 .And. "
//cFilter		+= "E1_TIPO == 'NF ' .AND. E1_PORTADO <> '  ' .AND. posicione('SA1',1,FWxFilial('SA1')+E1_CLIENTE+E1_LOJA,'A1_XBOLHIB') <> 'S' "
cFilter		+= "E1_TIPO == 'NF ' .AND. E1_PORTADO <> '  ' .AND. E1_XBOLHIB <> 'S' "
...
cNumTit:= SE1->E1_NUM  		//Numero do titulo
nbol2 				:= 0
dbGoTop()
ProcRegua(RecCount())
Do While !EOF()
	...
	IF cNumTit <> SE1->E1_NUM   //Numero do titulo
		cNumTit := SE1->E1_NUM  //Numero do titulo
		nBol    := 1
	...
	*---------------------------
	...
	If Subs(aDadosBanco[1],1,3) = "104" 										//Caixa econômica
		aCB_RN_NN    := RetBarCEF(E1_VALOR /*- E1_BONUS*/ - nVlrAbat, cNosNum)	//SE1->E1_NUMBCO)
	Elseif Subs(aDadosBanco[1],1,3) = "237" 									//Bradesco
		aCB_RN_NN    := Ret_237cBarra(Subs(aDadosBanco[1],1,3)+"9", ; 			//"0009"
		...
		(E1_VALOR - nVlrAbat)			,;
		SE1->E1_VENCTO      			,;
		" "								,;  									//nao utiliza no bradesco
		" "								,;  									//nao utilizado no bradesco
		Iif(SE1->E1_DECRESC > 0,.t.,.f.),;  									//nao utilizado no bradesco.
		SE1->E1_PARCELA      			,;
		aDadosBanco[3]       			,;
		cNosNum)//SE1->E1_NUMBCO)  												//nao utilizado, restante das informacoes.
	ELSE   //Banco do Brasil
		...
		cNroDoc		,(E1_VALOR /*- E1_BONUS*/ - nVlrAbat)	, "18"	,"9",cNosNum)
	...
	If Marked("E1_OK")
		IF nBol = 1
			...
			For nI := 100 to 2360 step 50
				oPrint:Line(1090, nI, 1090, nI+30)
				oPrint:Line(2260, nI, 2260, nI+30)
			Next nI
		Endif
...
For nI := nRow3 + 40 to nRow3+1050 step 30
...
oPrint:Line (nRow3+0140,100,nRow3+0140,450)

oPrint:Say  (nRow3+0150,130 ,"RECIBO DO PAGADOR",oFont8n)
oPrint:Line (nRow3+0190,100,nRow3+0190,450)
...
oPrint:Line (nRow3+0400,100,nRow3+0400,450)
...
oPrint:Line (nRow3+1000,100,nRow3+1000,450)
...
oPrint:Line (nRow3+1110,100,nRow3+1110,450)
...
oPrint:Say  (nRow3+0150,500 ,"Local de Pagamento",oFont8)
If Subs(aDadosBanco[1],1,3) <> "237"
	//oPrint:Say  (nRow3+0180,500 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO",oFont8n)
	oPrint:Say  (nRow3+0180,500 ,"PAGÁVEL EM QUALQUER BANCO",oFont8n)
ELSE
	oPrint:Say  (nRow3+0180,500 ,"Preferencialmente nas agencias do Bradesco ou Correios. APÓS O VENCIMENTO, SOMENTE NO BRADESCO",oFont8n) //Documento Não Compensável – Pagável Exclusivamente no Bradesco
ENDIF

oPrint:Say  (nRow3+0150,2010,"Vencimento",oFont8)
cString := StrZero(Day(aDadosTit[4]),2) +"/"+ StrZero(Month(aDadosTit[4]),2) +"/"+ Right(Str(Year(aDadosTit[4])),4)
nCol	 	 := 1930+(320-(len(cString)*22))
oPrint:Say  (nRow3+0190,nCol,cString,oFont10c)

oPrint:Say  (nRow3+0250,500 ,"Beneficiário",oFont8)
...
oPrint:Say  (nRow3+0250,2010,"Agência/Código Beneficiário",oFont8)
If Subs(aDadosBanco[1],1,3) =="104"
	cString := Alltrim(aDadosBanco[3]+"/"+aDadosBanco[4]+/*"-"+*/aDadosBanco[5])
ElseIF Subs(aDadosBanco[1],1,3) == "237"
	cString := SUBSTR(aDadosBanco[3],1,Len(AllTrim(aDadosBanco[3]))-1)+"-"+SUBSTR(aDadosBanco[3],Len(AllTrim(aDadosBanco[3])),1)+"/"+aDadosBanco[4]+/*"-"+*/aDadosBanco[5]
...
oPrint:Say  (nRow3+0350,1305,"Espécie Doc."                                   	,oFont8)
oPrint:Say  (nRow3+0380,1350,aDadosTit[8]										,oFont10) //Tipo do Titulo

oPrint:Say  (nRow3+0350,1555,"Aceite"                                         	,oFont8)
oPrint:Say  (nRow3+0380,1585,"N"                                             	,oFont10)

oPrint:Say  (nRow3+0350,1685,"Data do Processamento"                          	,oFont8)
...
oPrint:Say  (nRow3+0350,2010,"Nosso Número"                                   	,oFont8)
...
oPrint:Say  (nRow3+0420,500 ,"Uso do Banco"                                 	,oFont8)

If Subs(aDadosBanco[1],1,3) == "237"
	oPrint:Say  (nRow3+0420,805 ,"Cip"                                        	,oFont8)
	oPrint:Say  (nRow3+0450,805 ,"000"                                       	,oFont10)
Endif	

oPrint:Say  (nRow3+0420,905 ,"Carteira"                                     	,oFont8)
* Banco do_ brasil
If Subs(aDadosBanco[1],1,3) <> "104" .AND. Subs(aDadosBanco[1],1,3) <> "237"
...
oPrint:Say  (nRow3+0420,1155 ,"Espécie"                                     	,oFont8)
oPrint:Say  (nRow3+0450,1205 ,"R$"                                          	,oFont10)

oPrint:Say  (nRow3+0420,1305,"Quantidade"                                   	,oFont8)
oPrint:Say  (nRow3+0420,1685,"Valor"                                        	,oFont8)

oPrint:Say  (nRow3+0420,2010,"Valor do Documento"                          		,oFont8)
cString := Alltrim(Transform(aDadosTit[5],"@E 99,999,999.99"))
nCol 	 := 2010+(260-(len(cString)*22))
oPrint:Say  (nRow3+0450,nCol,cString,oFont10c)

oPrint:Say  (nRow3+0490,500 ,"Instruções (Todas informações deste bloqueto são de exclusiva responsabilidade do beneficiário)",oFont8n)
...
oPrint:Say  (nRow3+0490,2010,"(-)Desconto/Abatimento"                         ,oFont8)
oPrint:Say  (nRow3+0560,2010,"(-)Outras Deduções"                             ,oFont8)
oPrint:Say  (nRow3+0630,2010,"(+)Mora/Multa"                                  ,oFont8)
oPrint:Say  (nRow3+0700,2010,"(+)Outros Acréscimos"                           ,oFont8)
oPrint:Say  (nRow3+0770,2010,"(=)Valor Cobrado"                               ,oFont8)

oPrint:Say  (nRow3+0710,500 ,"Pagador"                                         ,oFont8)
oPrint:Say  (nRow3+0710,720 ,aDatSacado[1]+" ("+aDatSacado[2]+")"             ,oFont8)

if aDatSacado[8] = "J"
	oPrint:Say  (nRow3+0710,720 ,aDatSacado[1]+" ("+aDatSacado[2]+") CNPJ: "+TRANSFORM(aDatSacado[7],"@R 99.999.999/9999-99"),oFont8) // CGC
Else
	oPrint:Say  (nRow3+0710,720 ,aDatSacado[1]+" ("+aDatSacado[2]+") CPF: "+TRANSFORM(aDatSacado[7],"@R 999.999.999-99"),oFont8) 	// CPF
EndIf

oPrint:Say  (nRow3+0740,720 ,aDatSacado[3]                                    ,oFont8)
oPrint:Say  (nRow3+0770,720 ,aDatSacado[6]+"    "+aDatSacado[4]+" - "+aDatSacado[5],oFont8) // CEP+Cidade+Estado

oPrint:Say  (nRow3+0790,500 ,"Sacador/Avalista"                               ,oFont8)
oPrint:Say  (nRow3+0865,1950,"Ficha de Compensação" ,oFont8n)
oPrint:Say  (nRow3+0895,1950,"Autenticação no verso",oFont8n)
...
IF Subs(aDadosBanco[1],1,3) == "237"
   ...
   oPrint:Say  (nRow3+1050,1950,"BANCO DO BRASIL ",oFont12n)
endif

oPrint:Say  (nRow3+1110,0100,Replicate("-",200),oFont14n) //Adilson Jorge em 03/08/11
...
* Função   :  Ret_cBarra
*----------------------------------------------------------------------------
* Objetivo :  Gera o código de barras e a linha digitável para o Banco do_
*             Brasil
*----------------------------------------------------------------------------
Static Function Ret_cBarra(	cPrefixo	,cNumero	,cParcela	,cTipo	 ,;
cBanco		,cAgencia	,cConta		,cDacCC	 ,;
cNroDoc		,nValor		,cCart		,cMoeda, _xNumBco	)
...
Else
	NNUM         := cNosNum		//Nosso Numero
Endif

dbSelectArea("SE1")
cDigNosso  := U_CALC_dig(NNUM) 	//cDigNosso não está sendo usado
cNosso     := NNUM 				//+ cDigNosso

cCampoL    := "000000"+ALLTRIM(SA6->A6_AGENCIA)+NNUM+cCart
cAge       := ALLTRIM(SA6->A6_AGENCIA)  //Agencia bruno
nVa		   := nValor  			//Valor Bruno
cCon       := ALLTRIM(SA6->A6_NUMCON)
cVal       := ''
cBarnov    := ''
If nValor > 0
	cFatorValor  := u_fator1n()+strzero(nValor*100,10)
	
	cVal := strzero(nValor*100,10)
Else
	cFatorValor  := u_fator1n()+strzero((SE1->E1_VALOR) /*- SE1->E1_BONUS)*100*/,10)
	cVal := strzero((SE1->E1_VALOR),10)
Endif
...
* Função   :  Calc_Dig
*----------------------------------------------------------------------------
* Objetivo :  Calcula o digito verificador nosso número do_ banco do_ brasil
*----------------------------------------------------------------------------
...
* Função   :  DIG2n
*----------------------------------------------------------------------------
* Objetivo :  Calculo da linha digitavel do_ banco do_ brasil
*----------------------------------------------------------------------------
User Function DIG2n(cVariavel)
Local Auxi := 0, sumdig := 0
...
* Relatório : Fator1n
*----------------------------------------------------------------------------
* Objetivo :  Calculo do_ fator de vencimento da linha digitável
*----------------------------------------------------------------------------
User function Fator1n()
...
±±ºFuncao    ³CALC5p2   ºAutor  ³Microsiga           º Data ³  02/13/04   º±±
...
*/
User Function CALC5p2(cVariavel)
Local Auxi := 0, sumdig := 0
...
Local cNNumSDig		:= cCpoLivre := cCBSemDig := cNNum := cFatVenc := ''
Local cNossoNum
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programa  ³ Mod10 ³ Autor ³                       ³ Data ³ 13/10/03 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
*----------------------------------------------------------------------------
* Adilson Jorge                                                  | 28/11/2019
*----------------------------------------------------------------------------
* Função : mod10
*----------------------------------------------------------------------------
* Objetivo :  Calcular modulo10
*----------------------------------------------------------------------------

User Function Mod10(cData)
Local L,D,P := 0
Local B     := .F.
...
User Function TBOL011()
MsApp():New("SIGACTB")
oApp:CreateEnv()
PtSetTheme("OCEAN")
oApp:cStartProg := "U_BOL011"
SetFunName('U_BOL011')
oApp:lMessageBar:= .T.
__lInternet     := .T.
oApp:Activate()
