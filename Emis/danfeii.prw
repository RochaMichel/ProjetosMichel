...
#INCLUDE "TOPCONN.CH"	//NORDESTE ATACADO
#INCLUDE "TOTVS.CH"
...
#DEFINE HMARGEM    045 //NORDESTE ATACADO
#DEFINE VMARGEM    045 //NORDESTE ATACADO
#DEFINE MAXITEM    022                                                // Máximo de produtos para a primeira página
#DEFINE MAXITEMP2  050 //NORDESTE ATACADO                             // Máximo de produtos para a pagina 2 em diante
#DEFINE MAXITEMP2F 051                                                // Máximo de produtos para a página 2 em diante quando a página não possui informações complementares
...
#DEFINE MAXITEMC   045 //NORDESTE ATACADO                             // Máxima de caracteres por linha de produtos/serviços
...
//Ajustado o valor do parametro acima MAXVALORC de 008 para 014, Adilson Jorge em 10/08/2012, Nordeste Atacado
//para solucionar o problema de quebra de casas decimais para outra linha.

//#DEFINE MAXITEMP2  068 //NORDESTE ATACADO                             // Máximo de produtos para a pagina 2 em diante
...
local lJob		:= .F.
local cProg
cProg := iif(existBlock("DANFEProc"),"U_DANFEProc","DANFEProc")

Default lAutomato := .F.  //Adilson Jorge 01/02/2021 - NE Atacado
...
If nTipo <> 1
	...
	/**********************
	* Five Solutions - 01/07/2010
	* Variaveis para uso na impressao dos dados adicionais do DANFE NE Atacado
	******************************************************************************/	
	Private xTRIB_AUX := {}
	Private VRED6 := 0
	Private Vred3 := 0
	...
	If ( oDanfe:lInJob )
		...
		u_danfeproc( @oDanfe, Nil, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, Nil, lAutomato )
	...
	Else
		...
		RPTStatus( {|lEnd| u_danfeproc( @oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja,Nil, lAutomato )}, "Imprimindo DANFE..." )
...
//Incluido por Adilson Jorge em 26/07/2011, solicitado por Marcelo - NE ATACADO
//Alert("Gerou Danfe, posso chamar outra rotina, Adilson Jorge")
If Select("QRY") > 0
	QRY->(DBCloseArea())
Endif

cQuery := "SELECT QTD=COUNT(*) FROM "+RETSQLNAME("SE1")+" SE1 WITH (NOLOCK) " + CRLF
...
//Adição de filtros para trazer somente os títulos de clientes que não estão selecionados
//como parte do fluxo de Boleto Híbrido - 23/08/2023 Mateus Ramos
cQuery += "AND E1_XNUMBOL = '' "
//cQuery += " AND ( SELECT A1_XBOLHIB FROM "+RETSQLNAME("SA1")+" A1 "
//cQuery += " WHERE A1.D_E_L_E_T_ <> '*' "
//cQuery += " AND A1_COD = SE1.E1_CLIENTE "
//cQuery += " AND A1_LOJA = SE1.E1_LOJA ) <> 'S' "
cQuery += "AND D_E_L_E_T_ = ' ' "
// OLHAR PARA O CAMPO -> E1_XBOLHIB
dbUseArea(.T.,'TOPCONN', TCGenQry(,,cQuery), "QRY",.F.,.T.)

dbselectArea("QRY")

If QRY->QTD > 0  // Só vai perguntar se vai imprimir os boletos se os titulos forem boletos

	IF MsgYesNo("Imprime o(s) boleto(s) ? ")
		U_BOL011() //CHAMADA DO BOLETO BRADESCO A4
	Endif
...
Return lRet
/*/{Protheus.doc} BoletoBB
Função de controle de impressão e de registro no banco do boleto híbrido
@type function
@version 12.1.2210 
...
Static Function BoletoBB()
	...
	cQuery += " E1_EMISSAO, "
	cQuery += " E1_VENCREA, "
	cQuery += " E1_VALOR FROM "+RetSqlName('SE1')+" E1 "
	cQuery += " WHERE E1.E1_NUM BETWEEN '"+MV_PAR01+"' AND '"+MV_PAR02+"' "
	cQuery += " AND E1.E1_XNUMBOL = ' ' "
	cQuery += " AND E1.E1_TIPO = 'NF' "
	cQuery += " AND E1.E1_FILIAL = '"+FWxFilial('SE1')+"' "
	cQuery += " AND ( SELECT A1_XBOLHIB FROM "+RETSQLNAME("SA1")+" A1 "
	cQuery += " WHERE A1.D_E_L_E_T_ <> '*'  "
	cQuery += " AND A1_COD = E1.E1_CLIENTE  "
	cQuery += " AND A1_LOJA = E1.E1_LOJA ) = 'S' "
	cQuery += " AND  "
	cQuery += " ( ( SELECT COUNT(*) FROM "+RetSqlName('ZBH')+" ZBH "
	cQuery += " WHERE ZBH.ZBH_FILIAL = E1.E1_FILIAL  "
	cQuery += " AND ZBH.ZBH_TITULO = E1.E1_NUM AND ZBH.ZBH_PREFIX = E1.E1_PREFIXO "
	cQuery += " AND ZBH.ZBH_PARCEL = E1.E1_PARCELA AND ZBH.D_E_L_E_T_ <> '*' ) "
	cQuery += " = 0 "
	cQuery += " OR ( SELECT ZBH_FLAGPG FROM "+RetSqlName('ZBH')+" ZBH "
	cQuery += " WHERE ZBH.ZBH_FILIAL = E1.E1_FILIAL  "
	cQuery += " AND ZBH.ZBH_TITULO = E1.E1_NUM AND ZBH.ZBH_PREFIX = E1.E1_PREFIXO "
	cQuery += " AND ZBH.ZBH_PARCEL = E1.E1_PARCELA AND ZBH.D_E_L_E_T_ <> '*' "
	cQuery += " GROUP BY ZBH_FLAGPG) = ' ' ) "
	cQuery += " AND E1.D_E_L_E_T_ <> '*' "
	cQuery += " GROUP BY E1_FILIAL, E1_PREFIXO, E1_NUM, E1_PARCELA, E1_TIPO, E1_LOC, E1_CLIENTE, E1_LOJA, E1_EMISSAO, E1_VENCREA, E1_VALOR "
	
	MpSysOpenQuery(cQuery, cAliasTmp)
	DbSelectArea("SE1")
	SE1->(DbSetOrder(1))
	While (cAliasTmp)->(!EoF())
			If SE1->(DbSeek((cAliasTmp)->E1_FILIAL+(cAliasTmp)->E1_PREFIXO+(cAliasTmp)->E1_NUM+(cAliasTmp)->E1_PARCELA+(cAliasTmp)->E1_TIPO)) .AND. (cAliasTmp)->E1_LOC == "17"
				//cLog += Replicate("-",101) + ENTER + ENTER
				//cLog += "Banco: BB  -  Título: "+SE1->E1_NUM + ENTER + ENTER
				FwMsgRun(,{ || cLog += U_BB(1) }, 'Registrando Boleto(s)', 'Aguarde...')
			//ElseIf SE1->(DbSeek((cAliasTmp)->E1_FILIAL+(cAliasTmp)->E1_PREFIXO+(cAliasTmp)->E1_NUM+(cAliasTmp)->E1_PARCELA+(cAliasTmp)->E1_TIPO)) .AND. (cAliasTmp)->E1_LOC == "16"
			//	FwMsgRun(,{ || cLog += U_BRADESCO() + ENTER + ENTER },'Registrando Boleto(s)','Aguarde...')
			EndIf
		(cAliasTmp)->(DbSkip())
	End
	...
	cQuery := "SELECT * FROM "+RETSQLNAME("SE1")+" SE1 WITH (NOLOCK) " + CRLF
	cQuery += "WHERE E1_FILIAL = '"+xFilial("SE1")+"' " + CRLF
	cQuery += "AND E1_NUM BETWEEN '"+MV_PAR01+"' AND '"+MV_PAR02+"' " + CRLF
	cQuery += "AND E1_PORTADO <> '' " + CRLF
	cQuery += "AND E1_SALDO > 0 " + CRLF
	...
	cQuery += " AND  "
	cQuery += " ( SELECT COUNT(*) FROM "+RetSqlName('ZBH')+" ZBH "
	cQuery += " WHERE ZBH.ZBH_FILIAL = SE1.E1_FILIAL  "
	cQuery += " AND ZBH.ZBH_TITULO = SE1.E1_NUM AND ZBH.ZBH_PREFIX = SE1.E1_PREFIXO "
	cQuery += " AND ZBH.ZBH_PARCEL = SE1.E1_PARCELA AND ZBH.D_E_L_E_T_ <> '*' AND ZBH.ZBH_FLAGPG NOT IN ('P','B','E','R') "
	cQuery += " AND ZBH.ZBH_CODBAR <> ' ' ) "
	cQuery += " > 0 "
	cQuery += "AND D_E_L_E_T_ = ' ' "
	...
	If (cAliasTmp)->(!EoF()) 
 // Só vai perguntar se vai imprimir os boletos se os titulos forem boletos
		//IF MsgYesNo("Imprime o(s) boleto(s) ? ")
		If SF2->(DbSeek(FWxFilial('SF2')+(cAliasTmp)->E1_NUM+(cAliasTmp)->E1_SERIE))
			U_BOLHIB(MV_PAR01,MV_PAR02) //CHAMADA DO BOLETO BRADESCO A4
		EndIf
		//Endif
	Endif
	RestArea(aAreaF2)
...
User Function DANFEProc( oDanfe, lEnd, cIdEnt, cVal1, cVal2, lExistNfe, lIsLoja, nTipo, lAutomato )
...
local cProg
cProg		:= iif(existBlock("IMPDNFLJ"),"U_IMPDNFLJ","IMPDNFLJ")
...
//Adilson Jorge em 16/11/2021, NE Atacado
Local n_Stb 		:= 0 
Local Xtrib_aux 	:= {}
// Local nI,nJ,nX
...
//If !SF2->F2_DOC $ "000009149/000009150/000009151/000009152" //REATIVAR ESSE IF SE NAO QUISER IMPRIMIR OS DADOS FINANCEIROS, ADILSON EM 22/05/2012 NE ATACADO
...
If Type("oTransp:_Transporta")<>"U"
	...
	//Jussimar em 14/04/2011 as 11:11 retirado o comentário - NE ATACADO
	aTransp[02] := IIF(Type("oTransp:_ModFrete:TEXT")<>"U",oTransp:_ModFrete:TEXT,"0")
	// retorno da funcao GetDA3 (fica em SF2460I)
	// aRet[1]/*DA3->DA3_PLACA*/,aRet[2]/*DA3->DA3_ESTPLA*/,aRet[3]/*DA3->DA3_RNTC*/,aRet[4]/*DA3_TIPTRA*/
	aTransp[03] := IIf(Type("oTransp:_VeicTransp:_RNTC")=="U"      ,U_GetDA3()[3],oTransp:_VeicTransp:_RNTC:TEXT)
	aTransp[04] := IIf(Type("oTransp:_VeicTransp:_Placa:TEXT")=="U",U_GetDA3()[1],oTransp:_VeicTransp:_Placa:TEXT)
	aTransp[05] := IIf(Type("oTransp:_VeicTransp:_UF:TEXT")=="U"   ,U_GetDA3()[2],oTransp:_VeicTransp:_UF:TEXT)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
xTRIB_AUX := {} //Five Solutions - 09/07/2010 - NE ATACADO
// Jussimar 19/10/2010 as 12_41 - NE ATACADO
VRED6 := 0
Vred3 := 0
Vred4 := 0
Vred41 := 0
...
//Variáveis para tratar produtos tóxicos para abelhas, Adilson Jorge em 19/10/2012 NE Atacado
c_codtox := ""
n_codtox := 0
//---------------

//Variável para tratar produtos cujo o fornecedor seja Agroceres ou Biomatrix, Adilson Jorge em 31/01/2014 NE Atacado
n_codfor := 0
//---------------
...
For nZ := 1 To nLenDet
	...
	ne_prod  	:= Trim(oDet[nX]:_Prod:_cProd:TEXT) //Adilson Jorge em 14/04/2014 - NE Atacado
	nBaseICM 	:= 0
	nValICM  	:= 0
	nValIPI  	:= 0
	nPICM    	:= 0
	...
	//Jussimar - NE ATACADO
	nMaxDes := MaxCod(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte), 150)
	If nMaxDes < MAXITEMC 
	   nMaxDes := MAXITEMC
	EndIf
	...
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],2))))
   ...
   lPontilhado := .F.	//Adilson em 31/03/2015 - NE ATACADO
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
		...
		//Adilson em 10/02/2012 - NE ATACADO
	...
	If ( oDet[nX]:_Prod:_uTrib:TEXT <> oDet[nX]:_Prod:_uCom:TEXT )
		...
		aAuxCom := {}
		AADD(aAuxCom, AllTrim(TransForm(nQtdTrib,TM(nQtdTrib,TamSX3("D2_QUANT")[1],3))))
		AADD(aAuxCom, AllTrim(TransForm(nVUnitTrib,TM(nVUnitTrib,TamSX3("D2_PRCVEN")[1],2))))
	...
	// Jussimar em 25/03/2011 (Força a impresão do pontilhado) - NE ATACADO
	lPontilhado := .T.	
	...
	If (ValAtrib("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U" .Or. ValAtrib("oNf:_infnfe:_det:_Infadprod:TEXT") <> "U") .And. ( lImpAnfav .Or. lImpInfAd )
		...
		nMaxDes := Iif(nMaxDes<45,45,nMaxDes)// Solução para corrigir a quebra de linha na impressão do (FCP) dos itens. ( Rivaldo.JR | Cod.ERP | 09/08/2023 )
	...
	//Adilson Jorge em 03/07/2013, NE ATACADO
	//verificando se existe informações de FCI para imprimir nos detalhes do produto
	c_compl_fci := "" // Adilson Jorge em 03/07/2013, NE ATACADO
	//c_compl_fci := Alltrim(Posicione("SB5",1,xFilial("SB5")+Alltrim(ne_prod),"B5_CEME"))
	//a impressão é opcional, não é para imprimir e sim enviar na TAG do XML
	If !Empty(c_compl_fci) 
		cAux := c_compl_fci
		nMaxDesI := 041
		While !Empty(cAux)
			aadd(aItens,{;
				"",;
				"  "+SubStr(cAux,1,nMaxDesI),;
				"",;
				"",;
				"",;
				"",;
	...
	If lPontilhado
		//ADILSON JORGE EM 10/02/2012 - NE ATACADO
		aadd(aItens,{Replicate("- ",5),;   //6
		Replicate("- ",MAXITEMC),;
		Replicate("- ",12),;
		Replicate("- ",05),;
		Replicate("- ",05),; //07
...
//		})
	...
	/*****************
	* Five SOlutions - 01/07/2010
	* IMplementacao para Dados Adicionais
	* NE Atacado
	*****************************/
	cTab := Alias()
	cProdNE   := oDet[nX]:_Prod:_cProd:TEXT
	cCfopNE   := oDet[nX]:_Prod:_CFOP:TEXT  //Adilson Jorge em 15/10/2012
	cSB1_ICMS := Posicione("SB1",1,xFilial("SB1")+cProdNE,"B1_ICMS")
	//Adilson Jorge em 31/05/2013, Nordeste Atacado
	//cSB1_ORIGEM := Posicione("SB1",1,xFilial("SB1")+cProdNE,"B1_ORIGEM")

	//Adilson Jorge em 22/09/2021, Nordeste Atacado
   cSB1_ORIGEM := Substring(Posicione("SD2",3,xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA+cProdNE,"D2_CLASFIS"),1,1)

   //D2_FILIAL+D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_COD+D2_ITEM                                                                                                     

	//Adilson Jorge em 31/01/2014, Nordeste Atacado
	cSB1_CODFOR := Posicione("SB1",1,xFilial("SB1")+cProdNE,"B1_CODFOR")
	//------------------
	DbSelectArea(cTab)
	
	// Ricardo aqui 05/10/2010
	Private c_UF2     := Space(02)
	Private Xest_Cli  := Space(02)
	
	Xest_Cli := oDestino:_EnderDest:_UF:Text
	c_UF2    := GETMV("MV_ESTADO")    &&&   30.06.1999

	nAliqNE := 0

	If SF2->F2_EMISSAO <= CTOD("31/12/2015")	
		nAliqNE := 0.17
	Else 
		nAliqNE := 0.18
	Endif
				
	If cSB1_ICMS == "1"   // tipo=1 normal
		c_St := "00"
		If     cSB1_ORIGEM == "1"
			    c_St := "10"
  		ElseIf cSB1_ORIGEM == "2"
			    c_St := "20"
  		ElseIf cSB1_ORIGEM == "3"
			    c_St := "30"
  		ElseIf cSB1_ORIGEM == "5"
			    c_St := "50"
		ElseIf cSB1_ORIGEM == "6"
			    c_St := "60"
  		ElseIf cSB1_ORIGEM == "8"
			    c_St := "80"
		Endif
	ElseIf cSB1_ICMS == "2"  // // tipo=2 isento
		c_St := "02"
		VRED6:=0
	ElseIf cSB1_ICMS == "3"  // tipo=3 reduzindo
		If c_Uf2 == Xest_Cli        &&& Dentro do Estado
			c_St := "05D"
			If 	 cSB1_ORIGEM == "1"
					 c_St := "05D12"
  			ElseIf cSB1_ORIGEM == "2"
  			...
  			ElseIf cSB1_ORIGEM == "8"
				    c_St := "05D82"
			Endif
			//Ajustado por Adilson Jorge em 01/06/2016, para considerar a redução de base correta de acordo com a filial.
			//If Trim(sm0->m0_codfil) == "03"
			//	Vred3 := Vred3 + (.670588 * (nQtd * nVUnit)) *  nPICM/100
			//Else
			//	Vred3 := Vred3 + (.688889 * (nQtd * nVUnit)) *  nPICM/100
			//Endif
			Vred3 := Vred3 + (.688889 * (nQtd * nVUnit)) *  nPICM/100
		Else
			c_St := "05F"
			...
			ElseIf cSB1_ORIGEM == "6"
  			...
  			ElseIf cSB1_ORIGEM == "8"
				    c_St := "05F82"
			Endif
			Vred3 := Vred3 + (.416667 * (nQtd * nVUnit)) *  nPICM/100
//			If cCfopNE == "6552" //Se for transferencia de imobilizado, Adilson Jorge em 15/10/2012
			If cCfopNE $ "6552/6551" //Se for transferencia de imobilizado, Adilson Jorge em 15/10/2012
				c_stNE := .T.
			Endif	
		Endif
	ElseIf cSB1_ICMS == "4" // tipo=4 diferido
		If c_Uf2 == Xest_Cli        &&& Dentro do Estado
			//Crítica inserida para controlar por parâmetro a mensagem de isento para base reduzida na filial 08 - Sergipe
			//Adilson Jorge em 31/08/2016. Lembrar de rever os pedidos em aberto no sistema em Sergipe, mudar o TES de 560/E60 para 
			If GETMV("MV_NEICM08")=="S" // #PESQTES
				//Crítica inserida para mudar a mensagem de isento para base reduzida na filial 08 - Sergipe
				//Adilson Jorge em 05/08/2015.
				If Trim(sm0->m0_codfil) <> "08"
					c_St := "04"
					If     cSB1_ORIGEM == "1"
						c_St := "14"
					ElseIf cSB1_ORIGEM == "2"
					...
					ElseIf cSB1_ORIGEM == "8"
						c_St := "84"
					Endif
					Vred4 := Vred4 + (nAliqNE * (nQtd * nVUnit))
					If cSb1_CodFor == "53"
						c_St := "X8"
						Vred4 := Vred4 + (nAliqNE * (nQtd * nVUnit))
					Endif
				Else
					//Adilson Jorge em 11/09/2015
					c_St := "A8"
					If     cSB1_ORIGEM == "1"
						    c_St := "18" //"18"
	   			ElseIf cSB1_ORIGEM == "2"
   				...
   				ElseIf cSB1_ORIGEM == "8"
						    c_St := "88" //"88"
					Endif
					Vred4 := Vred4 + (.7647 * (nQtd * nVUnit)) *  nPICM/100
				Endif
			Else		//Se o MV_NEICM08 estiver com "N" // #PESQTES
				c_St := "04"
				If     cSB1_ORIGEM == "1"
					    c_St := "14"
			       	 //Vred41 := Vred41 + (0.04 * (nQtd * nVUnit))
			       	 Vred41 := Vred41 + (0.04 * ((nQtd * nVUnit)/(1-0.04)) )
   			ElseIf cSB1_ORIGEM == "2"
					    c_St := "24"
			       	 Vred42 := Vred42 + (0.04 * ((nQtd * nVUnit)/(1-0.04)) )
  				ElseIf cSB1_ORIGEM == "3"
					    c_St := "34"
			       	 Vred43 := Vred43 + (nAliqNE * ((nQtd * nVUnit)/(1-nAliqNE)) )
  				ElseIf cSB1_ORIGEM == "5"
					    c_St := "54"
			       	 Vred45 := Vred45 + (nAliqNE * ((nQtd * nVUnit)/(1-nAliqNE)) )
  				ElseIf cSB1_ORIGEM == "6"
					    c_St := "64"
			       	 Vred46 := Vred46 + (0.04 * ((nQtd * nVUnit)/(1-0.04)) )
  				ElseIf cSB1_ORIGEM == "7"
					    c_St := "74"
			       	 Vred47 := Vred47 + (0.04 * ((nQtd * nVUnit)/(1-0.04)) )
  				ElseIf cSB1_ORIGEM == "8"
					    c_St := "84"
			        	 Vred48 := Vred48 + (nAliqNE * ((nQtd * nVUnit)/(1-nAliqNE)) )
			   ElseIf cSB1_ORIGEM == "0"
						 Vred4 := Vred4 + (nAliqNE * ((nQtd * nVUnit)/(1-nAliqNE)) )
				Endif
				//Vred4 := Vred4 + (nAliqNE * (nQtd * nVUnit))
				If cSb1_CodFor == "53"
					c_St := "X8"
		...
		Else
			...
			If     cSB1_ORIGEM == "1"
				    c_St := "A0" //"18"
//			       Vred4a := Vred4a + (0.04 * (nQtd * nVUnit))
   		ElseIf cSB1_ORIGEM == "2"
				    c_St := "B0" //"28"
//			       Vred4b := Vred4b + (0.04 * (nQtd * nVUnit))
   		ElseIf cSB1_ORIGEM == "3"
				    c_St := "C0" //"38"
//			       Vred4c := Vred4c + (nAliqNE * (nQtd * nVUnit))
  			ElseIf cSB1_ORIGEM == "5"
				    c_St := "D0"
//			       Vred4d := Vred4d + (nAliqNE * (nQtd * nVUnit))
			ElseIf cSB1_ORIGEM == "6"
				    c_St := "E0"
//			       Vred4e := Vred4e + (0.04 * (nQtd * nVUnit))
			ElseIf cSB1_ORIGEM == "7"
				    c_St := "F0"
//			       Vred4f := Vred4f + (0.04 * (nQtd * nVUnit))
   		ElseIf cSB1_ORIGEM == "8"
				    c_St := "G0" //"88"
//			       Vred4g := Vred4g + (nAliqNE * (nQtd * nVUnit))
//		   ElseIf cSB1_ORIGEM == "0"
//					 Vred4 := Vred4 + (nAliqNE * (nQtd * nVUnit))
			Endif
			Vred4 := Vred4 + (.60 * (nQtd * nVUnit)) *  nPICM/100
			If cSb1_CodFor == "53"
				c_St := "X8"
				Vred4 := Vred4 + (nAliqNE * (nQtd * nVUnit))
			Endif
		Endif
	ElseIf cSB1_ICMS == "5" // tipo=5 diferido fertilizantes
		If c_Uf2 == Xest_Cli        &&& Dentro do Estado
			c_St := "04"
			If     cSB1_ORIGEM == "1"
				    c_St := "14"
		...
		Else
			c_St := "06"
			Vred5 := Vred5 + (.30 * (nQtd * nVUnit))  * nPICM/100
		Endif

		//If c_Uf2 == Xest_Cli        &&& Dentro do Estado
		//	c_St := "07"
		//	Vred5 := Vred5 + (nAliqNE * (nQtd * nVUnit))
		//Else
		//	c_St := "06"
		//	Vred5 := Vred5 + (.30 * (nQtd * nVUnit))  * nPICM/100
		//Endif
	EndIf
	If Ascan(xTRIB_AUX, c_St)==0
		AADD(xTRIB_AUX ,c_St)
	Endif
	//---------------------

	//Tratar produtos tóxicos para abelhas, Adilson Jorge em 19/10/2012 NE Atacado
	If cProdNe $ GETMV("MV_TOXABEL") .And. !cCfopNE $ "5152.6152"
	   c_codtox += Trim(cProdNe)+", "
		n_codtox += 1
	Endif	
	//-----------------

	//Tratar produtos cujo fornecedor seja Agroceres ou Biomatrix para imprimir RENASEM do CE, Adilson Jorge em 31/01/2014 NE Atacado
	If Trim(cSB1_CODFOR) $ "05/62"
		n_codfor += 1
	Endif	
	//-----------------
...
// Jussimar em 22/03/2011 as 09:48 - NE ATACADO
//---------------------------
Asort (xTRIB_Aux)
/*
cAux := '020-TRIBUTACAO COM REDUÇÃO DE BASE DE CÁLCULO CONF.DEC.14876/91 ART.24 INC.II'
aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
cAux := 'Mercedes Bens/A-190, gasolina, Ano 2002, Modelo 2003, 125CV, Cor Prata'
aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
cAux := ',Placa KHM 6437,Chassi 9BMMF32EX3A043168'
aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
*/

//adilson jorge 23/03/15 retirar depois
cDocTMP := StrZero(Val(oIdent:_NNf:Text),9)
cSerTMP := StrZero(Val(oIdent:_Serie:Text),3)
cQryXX := "SELECT SD2.D2_CLIENTE,SD2.D2_LOJA,SD2.D2_PEDIDO,SD2.D2_EMISSAO " + CRLF
cQryXX += "FROM "+RetSQLName("SD2")+" SD2 WITH (NOLOCK) " + CRLF
cQryXX += "WHERE SD2.D2_FILIAL = '"+xFilial("SD2")+"' " + CRLF
cQryXX += "AND SD2.D2_DOC = '"+cDocTMP+"' " + CRLF
cQryXX += "AND SD2.D2_SERIE = '"+cSerTMP+"' " + CRLF
cQryXX += "AND SD2.D_E_L_E_T_ = ' ' "
TCQuery cQryXX NEW ALIAS "TSD2"
DbSelectArea("TSD2")
cCodCli_LojaXX := TSD2->D2_CLIENTE + TSD2->D2_LOJA
cPedidoXX      := TSD2->D2_PEDIDO
cEmissaoXX     := TSD2->D2_EMISSAO
TSD2->(DbCloseArea())

cQryXX := "SELECT SC5.C5_TPSAIDA " + CRLF
cQryXX += "FROM "+RetSQLName("SC5")+" SC5 WITH (NOLOCK) " + CRLF
cQryXX += "WHERE SC5.C5_FILIAL = '"+xFilial("SC5")+"' " + CRLF
cQryXX += "AND SC5.C5_NUM = '"+cPedidoXX+"' " + CRLF
cQryXX += "AND SC5.D_E_L_E_T_ = ' ' "
TCQuery cQryXX NEW ALIAS "TSC5"
DbSelectArea("TSC5")
cTpSaidaXX := TSC5->C5_TPSAIDA
TSC5->(DbCloseArea())

cEmissaoXX := Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",ConvDate(oIdent:_DHEmi:TEXT),ConvDate(oIdent:_DEmi:TEXT))
...
//Adilson Jorge em 31/07/2017 - a partir de 01/08/2017 essas informações vão no XML pelo NFESEFAZ
//Alert(cEmissaoXX)
If CTOD(cEmissaoXX) <= CTOD("31/07/2017") //"20170731"

For n_Stb := 1 TO Len(Xtrib_aux)
	If Xtrib_aux [n_Stb] == "00"
		cAux := '000-TRIBUTACAO NORMAL'
	ElseIf Xtrib_aux [n_Stb] == "10" //Adilson Jorge em 31/05/2013
		cAux := '100-TRIBUTACAO NORMAL - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	//Incluído por Adilson Jorge em 08/04/2014
	ElseIf Xtrib_aux [n_Stb] $ "20" //"05F22"
		cAux := '200-TRIBUTACAO NORMAL - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] $ "30" //"05F32"
		cAux := '300-TRIBUTACAO NORMAL - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] $ "50" //"05F32"
		cAux := '500-TRIBUTACAO NORMAL - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] $ "60" //"05F32"
		cAux := '600-TRIBUTACAO NORMAL - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] $ "80" //"05F82"
		cAux := '800-TRIBUTACAO NORMAL - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	//----------------------------------------
	ElseIf Xtrib_aux [n_Stb] == "02"
		cAux := '020-BC RED.60% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "04"
		cAux := '040-ISENTO C.ICMS 100/97 DE 21/10/97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	//Incluído por Adilson Jorge em 03/11/2015
	ElseIf Xtrib_aux [n_Stb] == "X8"
		cAux := '041-NAO TRIBUTADA, SEM INCIDENCIA DO ICMS' //+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	//Incluído por Adilson Jorge em 08/04/2014
	ElseIf Xtrib_aux [n_Stb] == "14"
		cAux := '140-ISENTO C.ICMS 100/97 DE 21/10/97,VL.DISP.R$ '+ALLTRIM(Transform(VRED41,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "24"
		cAux := '240-ISENTO C.ICMS 100/97 DE 21/10/97,VL.DISP.R$ '+ALLTRIM(Transform(VRED42,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "34"
		cAux := '340-ISENTO C.ICMS 100/97 DE 21/10/97,VL.DISP.R$ '+ALLTRIM(Transform(VRED43,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "54"
		cAux := '540-ISENTO C.ICMS 100/97 DE 21/10/97,VL.DISP.R$ '+ALLTRIM(Transform(VRED45,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "64"
		cAux := '640-ISENTO C.ICMS 100/97 DE 21/10/97,VL.DISP.R$ '+ALLTRIM(Transform(VRED46,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "74"
		cAux := '740-ISENTO C.ICMS 100/97 DE 21/10/97,VL.DISP.R$ '+ALLTRIM(Transform(VRED47,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "84"
		cAux := '840-ISENTO C.ICMS 100/97 DE 21/10/97,VL.DISP.R$ '+ALLTRIM(Transform(VRED48,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	//----------------------------------------
	ElseIf Xtrib_aux [n_Stb] == "07"
		cAux := '040-ISENTO C.ICMS 100/97 DE 21/10/97,VL.DISP.R$ '+ALLTRIM(Transform(VRED5,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "05D"
		//Ajustado por Adilson Jorge em 01/06/2016, para considerar a redução de base correta de acordo com a filial.
		//If Trim(sm0->m0_codfil) == "03"
		//	cAux := '020-BC RED.32,9412% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))
		//Else
		//	cAux := '020-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))
		//Endif
		cAux := '020-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "05D12"
		//Ajustado por Adilson Jorge em 01/06/2016, para considerar a redução de base correta de acordo com a filial.
		//If Trim(sm0->m0_codfil) == "03"
		//	cAux := '120-BC RED.32,9412% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
		//Else
		//	cAux := '120-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
		//Endif
		cAux := '120-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	//Incluído por Adilson Jorge em 08/04/2014
	ElseIf Xtrib_aux [n_Stb] == "05D22"
		//Ajustado por Adilson Jorge em 01/06/2016, para considerar a redução de base correta de acordo com a filial.
		//If Trim(sm0->m0_codfil) == "03"
		//	cAux := '220-BC RED.32,9412% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
		//Else
		//	cAux := '220-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
		//Endif			
		cAux := '220-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "05D32"
		//Ajustado por Adilson Jorge em 01/06/2016, para considerar a redução de base correta de acordo com a filial.
		//If Trim(sm0->m0_codfil) == "03"
		//	cAux := '320-BC RED.32,9412% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
		//Else
		//	cAux := '320-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
		//Endif
		cAux := '320-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "05D52"
		//Ajustado por Adilson Jorge em 01/06/2016, para considerar a redução de base correta de acordo com a filial.
		//If Trim(sm0->m0_codfil) == "03"
		//	cAux := '520-BC RED.32,9412% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
      //Else
		//	cAux := '520-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
      //Endif			
		cAux := '520-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "05D62"
		//Ajustado por Adilson Jorge em 01/06/2016, para considerar a redução de base correta de acordo com a filial.
		//If Trim(sm0->m0_codfil) == "03"
		//	cAux := '520-BC RED.32,9412% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
      //Else
		//	cAux := '520-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
      //Endif			
		cAux := '620-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	ElseIf Xtrib_aux [n_Stb] == "05D82"
		//Ajustado por Adilson Jorge em 01/06/2016, para considerar a redução de base correta de acordo com a filial.
		//If Trim(sm0->m0_codfil) == "03"
		//	cAux := '820-BC RED.32,9412% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
		//Else
		//	cAux := '820-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
		//Endif			
		cAux := '820-BC RED.31,1111% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99')) +' - PRODUTO IMPORTADO, Resolucao SF 13/2012.'
	//----------------------------------------

	ElseIf Xtrib_aux [n_Stb] == "05F"
		cAux := '020-BC RED.58,3333% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))
		If c_stNE
			cAux := '020-BC RED. DE ACORDO COM INCISO II, ARTIGO 24, DECRETO 14.876/1991 DO RICMS/PE'
		Endif
	ElseIf Xtrib_aux [n_Stb] == "05F12"
		cAux := '120-BC RED.58,3333% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))+'. '
	ElseIf Xtrib_aux [n_Stb] == "05F22"
		cAux := '220-BC RED.58,3333% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))+'. '
	ElseIf Xtrib_aux [n_Stb] == "05F32"
		cAux := '320-BC RED.58,3333% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))+'. '
	ElseIf Xtrib_aux [n_Stb] == "05F52"
		cAux := '520-BC RED.58,3333% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))+'. '
	ElseIf Xtrib_aux [n_Stb] == "05F62"
		cAux := '620-BC RED.58,3333% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))+'. '
	ElseIf Xtrib_aux [n_Stb] == "05F82"
		cAux := '820-BC RED.58,3333% C.ICMS 52/91,26.09.91,VL.DISP.R$ '+ALLTRIM(Transform(VRED3,'@E 99,999.99'))+'. '

	ElseIf Xtrib_aux [n_Stb] == "06"
		cAux := '020-BC RED.30% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED5,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "08"
		cAux := '020-BC RED.60% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "A0"
		cAux := '120-BC RED.60% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4a,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "B0"
		cAux := '220-BC RED.60% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4b,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "C0"
		cAux := '320-BC RED.60% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4c,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "D0"
		cAux := '520-BC RED.60% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4d,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "E0"
		cAux := '620-BC RED.60% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4e,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "F0"
		cAux := '720-BC RED.60% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4f,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "G0"
		cAux := '820-BC RED.60% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4g,'@E 99,999.99'))
	//Adilson Jorge em 11/09/2015, ajustes para as NFs de Sergipe, produtos diferidos tipo 4
	ElseIf Xtrib_aux [n_Stb] == "A8"
		cAux := '020-BC RED.76,47% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "18"
		cAux := '120-BC RED.76,47% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "28"
		cAux := '220-BC RED.76,47% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "38"
		cAux := '320-BC RED.76,47% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "58"
		cAux := '520-BC RED.76,47% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "68"
		cAux := '620-BC RED.76,47% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	ElseIf Xtrib_aux [n_Stb] == "88"
		cAux := '820-BC RED.76,47% C.ICMS 100/97,21.10.97,VL.DISP.R$ '+ALLTRIM(Transform(VRED4,'@E 99,999.99'))
	//----------------------------------------
	Endif
	While !Empty(cAux)
//	While !Empty(cAux) .AND. cTpSaidaxx <> "000013" //Adilson Jorge 23/03/2015, coloquei dessa forma para não imprimir as informaçoes acima
...
cAux := "Produtos transportados conforme CAP.II,SECAO VI,ART.22,DEC.96044/88.Nr da ONU e classe de produtos mencionados fichas de emergencia."
//cAux := 'NF EMITIDA PARA EFEITO DE TRANSFERENCIA DE CREDITO DE ICMS NO VALOR DE R$ 30.000,00'
While !Empty(cAux)
	aadd(aMensagem, { SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30) - 1, MAXMENLIN+30)),.F.})
	cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30), MAXMENLIN+30) + 1)
EndDo
cAux := "Declaramos que os produtos constantes nesta nf estao adequadamente acondicionados para suportar "
//cAux := 'CONFORME O DEC. 14.876/91, ART. 51 § 1º E § 3º, II.'
While !Empty(cAux)
	aadd(aMensagem, { SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30) - 1, MAXMENLIN+30)),.F.})
	cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30), MAXMENLIN+30) + 1)
EndDo
cAux := "riscos normais de carregamento, descarregamento, transbordo e transporte conforme regulamento em vigor."
While !Empty(cAux)
	aadd(aMensagem, { SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30) - 1, MAXMENLIN+30)),.F.})
	cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30), MAXMENLIN+30) + 1 ) 
EndDo
//aAdd(aMensagem, "") //Linha em branco retirada por Adilson Jorge em 10/08/2012
//cAux := "Código do Cliente : "+SF2->F2_CLIENTE+"-"+SF2->F2_LOJA+"    |    Número do Pedido de Venda : "+SD2->D2_PEDIDO+"     |    Código do Vendedor : "+SF2->F2_VEND1
cAux := SF2->F2_CLIENTE+"-"+SF2->F2_LOJA+"    |    "+SD2->D2_PEDIDO+"     |    "+SF2->F2_VEND1
While !Empty(cAux)
	aadd(aMensagem, { SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30) - 1, MAXMENLIN+30)),.F.})
	cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30), MAXMENLIN+30) + 1 )      
EndDo
...
//--------------------------------------------------------
// Pega o Numero do Pedido de Compra se for Transferencia
// Jussimar em 21/01/2011 as 10:57 - NE ATACADO
//--------------------------------------------------------
cNumPedCompra := ""
cDocTMP := StrZero(Val(oIdent:_NNf:Text),9)
cSerTMP := StrZero(Val(oIdent:_Serie:Text),3)

cQryXX := "SELECT SD2.D2_CLIENTE,SD2.D2_LOJA,SD2.D2_PEDIDO " + CRLF
cQryXX += "FROM "+RetSQLName("SD2")+" SD2 WITH (NOLOCK) " + CRLF
cQryXX += "WHERE SD2.D2_FILIAL = '"+xFilial("SD2")+"' " + CRLF
cQryXX += "AND SD2.D2_DOC = '"+cDocTMP+"' " + CRLF
cQryXX += "AND SD2.D2_SERIE = '"+cSerTMP+"' " + CRLF
...
TSC5->(DbCloseArea())
cFILIALXX := cFilAnt
If cTpSaidaXX == "000012"
	If cCodCli_LojaXX == "00000101"
		cFILIALXX := "01"
	EndIf
	If cCodCli_LojaXX == "00583401"
		cFILIALXX := "03"
	EndIf
	If cCodCli_LojaXX == "00846001"
		cFILIALXX := "04"
	EndIf
	If cCodCli_LojaXX == "00583901"
		cFILIALXX := "05"
	EndIf
	If cCodCli_LojaXX == "00583601"
		cFILIALXX := "06"
	EndIf
	If cCodCli_LojaXX == "00583801"
		cFILIALXX := "07"
	EndIf
	If cCodCli_LojaXX == "00584001"  // ALTERADO POR MARCELO DIA 16/04/2012 INCLUINDO NOVAS FILIAIS 
		cFILIALXX := "08"
	EndIf
	If cCodCli_LojaXX == "00584101"  // ALTERADO POR MARCELO DIA 16/04/2012 INCLUINDO NOVAS FILIAIS SE = 08 E JUA = 09
		cFILIALXX := "09"
	EndIf
	If cCodCli_LojaXX == "00584701"  // ALTERADO POR ADILSON DIA 18/10/2016 INCLUINDO NOVA FILIAL PI = 10
		cFILIALXX := "10"
	EndIf
	If cCodCli_LojaXX == "00584901"  // ALTERADO POR ADILSON DIA 23/12/2020 INCLUINDO NOVA FILIAL CO = 11
		cFILIALXX := "11"
	EndIf
	
	cQryXX := "SELECT TOP 1 SC7.C7_NUM, SC7.C7_NFSERIE " + CRLF
	cQryXX += "FROM "+RetSQLName("SC7")+" SC7 WITH (NOLOCK) " + CRLF
	cQryXX += "WHERE SC7.C7_FILIAL = '"+cFILIALXX+"' " + CRLF
	cQryXX += "AND LEFT(SC7.C7_NFSERIE,12) = '"+cDocTMP+cSerTMP+"' " + CRLF
	cQryXX += "AND SC7.D_E_L_E_T_ = '' "
	TCQuery cQryXX NEW ALIAS "TSC7"
	DbSelectArea("TSC7")
	If Len(Alltrim(TSC7->C7_NUM)) > 0
		cNumPedCompra := "Número do Pedido de Compra : "+TSC7->C7_NUM
	EndIf
	TSC7->(DbCloseArea())
EndIf
//----------------------------------------------------------
...
If !Empty(cCodAutSef) .AND. oIdent:_tpEmis:TEXT<>"4"
	...
	// Jussimar em 21/01/2011 as 10:57 - NE ATACADO
	cAux := "Protocolo: "+cCodAutSef+IIF(Len(Alltrim(cNumPedCompra))>0,space(10)+cNumPedCompra,"")
	While !Empty(cAux)
		aadd(aMensagem, { SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)) , .F. } )
		cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
	EndDo
...
//Adilson Jorge em 18/10/2012 - NE Atacado
//c_codtox += Trim(cProdNe)+", "
If !Empty(c_codtox)
	If n_codtox == 1
		cAux := "(**)IMPORTANTE-O PRODUTO "+c_codtox+"É TÓXICO PARA ABELHAS. A APLICAÇÃO AÉREA NÃO É PERMITIDA. NÃO APLIQUE ESTE PRODUTO EM ÉPOCA DE FLORAÇÃO,NEM IMEDIATAMENTE ANTES DO FLORESCIMENTO OU QUANDO FOR OBSERVADA VISITAÇÃO DE ABELHAS NA CULTURA. O DESCUMPRIMENTO DESSAS DETERMINAÇÕES CONSTITUI CRIME AMBIENTAL, SUJEITO A PENALIDADES."
	ElseIf n_codtox > 1
		cAux := "(**)IMPORTANTE-OS PRODUTOS "+c_codtox+"SÃO TÓXICOS PARA ABELHAS. A APLICAÇÃO AÉREA NÃO É PERMITIDA. NÃO APLIQUE ESTE PRODUTO EM ÉPOCA DE FLORAÇÃO,NEM IMEDIATAMENTE ANTES DO FLORESCIMENTO OU QUANDO FOR OBSERVADA VISITAÇÃO DE ABELHAS NA CULTURA. O DESCUMPRIMENTO DESSAS DETERMINAÇÕES CONSTITUI CRIME AMBIENTAL, SUJEITO A PENALIDADES."
	Endif
	While !Empty(cAux)
		//aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30) - 1, MAXMENLIN+30)))
		//cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN+30) > 1, EspacoAt(cAux, MAXMENLIN+30), MAXMENLIN+30) + 1)
		aadd(aMensagem, { SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN+10) > 1, EspacoAt(cAux, MAXMENLIN+10) - 1, MAXMENLIN+10)),.F.})
		cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN+10) > 1, EspacoAt(cAux, MAXMENLIN+10), MAXMENLIN+10) + 1)
	EndDo
Endif	
//-------------

//Adilson Jorge em 31/01/2014 - NE Atacado
//Se o fornecedor de algum produto for Agroceres ou Biomatrix para imprimir RENASEM nas NFs do CE
If Trim(sm0->m0_codfil) == "03"
	If n_codfor > 0
		cAux := "RENASEM NR. CE-03002/2014."
		While !Empty(cAux)
			aadd(aMensagem, { SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN+10) > 1, EspacoAt(cAux, MAXMENLIN+10) - 1, MAXMENLIN+10)),.F.})
			cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN+10) > 1, EspacoAt(cAux, MAXMENLIN+10), MAXMENLIN+10) + 1)
		EndDo
...
If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
	...
	While !Empty(cAux)
		...
		//Adilson Jorge em 28/07/2017 - NE Atacado, acrescentar +10 na variável MAXMENLIN nas linhas abaixo, para usar o máximo da linha
		aadd(aMensagem, { SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN+10) > 1, EspacoAt(cAux, MAXMENLIN+10) - 1, MAXMENLIN+10)),.F.})
		cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN+10) > 1, EspacoAt(cAux, MAXMENLIN+10), MAXMENLIN+10) + 1)
		//-------------
...
*/
	nFolhas	  := 1
	//------------------------------------------------------------
	// Jussimar em 31/03/2011 as 16:25 - NE ATACADO
	// Evita Gerar uma folha imprima apenas os pontilhados
	//------------------------------------------------------------
	aTmp := aClone(aItens)
	If (Len(aItens) = (MAXITEM +1)) .and. ("- - - - - - - - -"$aItens[(MAXITEM +1)][1])
		aItens := {}
		For nI := 1 to (MAXITEM)
			AADD(aItens,{,,,,,,,,,,,,,})
			For nJ := 1 to 14
				aItens[nI][nJ] := aTmp[nI][nJ]
			Next
		Next
		aTmp := {}
	EndIf
...
If nfolha==1
	If lMv_Logod
		...
		oDanfe:SayBitmap(042,000,cLogoD,095,043)
	Else
		//oDanfe:SayBitmap(045,001,cLogo,080,090)
		oDanfe:SayBitmap(042,000,cLogo,095,043)
	EndIF
Endif
...
//Jussimar em 25/03/2011 as 20:46 - NE ATACADO
//If cMVCODREG $ "2|3" 
...
* 20/07/2010 - NE ATACADO
* Querys para Calculos dos
* Pesos Bruto e Líquido.
********************************/
//cQryPB := " SELECT SF2.F2_PLIQUI PBRUTO "
cQryPB := "SELECT SF2.F2_PBRUTO PBRUTO " + CRLF    //USAR ESSA LINHA QUANDO FOR NECESSÁRIO IMPRIMIR O F2_BRUTO, ADILSON 27/03/2019
cQryPB += "FROM "+RetSQLName("SF2")+" SF2 WITH (NOLOCK) " + CRLF
cQryPB += "WHERE SF2.F2_FILIAL = '"+xFilial("SF2")+"' " + CRLF
cQryPB += "AND SF2.F2_DOC = '"+cDocTMP+"' " + CRLF
cQryPB += "AND SF2.F2_SERIE = '"+cSerTMP+"' " + CRLF
cQryPB += "AND SF2.D_E_L_E_T_ = ' ' "

TCQuery cQryPB NEW ALIAS "TSF2"
TCSetField("TSF2","PBRUTO","N",17,04)
DbSelectArea("TSF2")
nPBrutNF := TSF2->PBRUTO
DbCloseArea()

cQryPB := "SELECT SUM(SD2.D2_QUANT * SB1.B1_PESO) PLIQUI " + CRLF
cQryPB += "FROM "+RetSQLName("SD2")+" SD2 WITH (NOLOCK) ,"+RetSQLName("SB1")+" SB1 WITH (NOLOCK) " + CRLF
cQryPB += "WHERE SD2.D2_FILIAL = '"+xFilial("SD2")+"' " + CRLF
cQryPB += "AND SB1.B1_FILIAL = '"+xFilial("SB1")+"' " + CRLF
cQryPB += "AND SD2.D2_COD = SB1.B1_COD " + CRLF
cQryPB += "AND SD2.D2_DOC = '"+cDocTMP+"' " + CRLF
cQryPB += "AND SD2.D2_SERIE = '"+cSerTMP+"' " + CRLF
cQryPB += "AND SD2.D_E_L_E_T_ = ' ' " + CRLF
cQryPB += "AND SB1.D_E_L_E_T_ = ' ' " + CRLF

//MemoWrite("PesoLiquidoDANFE.SQL",cQryPB)
TCQuery cQryPB NEW ALIAS "TSD2"
TCSetField("TSD2","PLIQUI","N",17,04)
DbSelectArea("TSD2")
//nPLiquiNF := Transform(TSD2->PLIQUI,"@E 999,999,999.9999")
nPLiquiNF := TSD2->PLIQUI
DbCloseArea()
//------------
...
ElseIf cModFrete =="1"
//	oDanfe:Say(382,247,"1-DESTINATARIO",oFont08:oFont) //ADILSON JORGE EM 10/02/2012 - NE ATACADO
...
//Adilson Jorge em 03/03/2020 - NE ATACADO
...
//oDanfe:Say(428,402,Iif (!Empty(aEspVol[1][3]),Transform(val(aEspVol[1][3]),"@E 999999.9999"),""),oFont08:oFont)
oDanfe:Say(428,402,Transform(nPBrutNF,"@E 999999.9999"),oFont08:oFont)
...
//oDanfe:Say(428,502,Iif (!Empty(aEspVol[1][2]),Transform(val(aEspVol[1][2]),"@E 999999.9999"),""),oFont08:oFont)
oDanfe:Say(428,502,Transform(nPLiquiNF,"@E 999999.9999"),oFont08:oFont)
//------
...
//Jussimar em 07/04/2011 as 09:06 - NE ATACADO
oDanfe:Say(440,002,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont)
oDanfe:Box(442,000,678,543)//603
//oDanfe:Box(442,000,678,025) //052
//oDanfe:Say(450,002,"COD. PROD",oFont08N:oFont)
oDanfe:Say(450,002,"COD.",oFont08N:oFont)
//oDanfe:Box(442,052,678,155)
//oDanfe:Say(450,054,"DESCRIÇÃO DO PROD./SERV.",oFont08N:oFont)
//oDanfe:Box(442,155,678,190)
oDanfe:Box(442,22,678,219)
oDanfe:Say(450,028,"DESCRIÇÃO DO PROD./SERV.",oFont08N:oFont)  //030
oDanfe:Say(450,226,"NCM/SH",oFont08N:oFont)
//oDanfe:Box(442,190,678,210)
//oDanfe:Say(450,192,"CST",oFont08N:oFont)
oDanfe:Box(442,270,678,254)   //230
oDanfe:Say(450,257,"CST",oFont08N:oFont)
oDanfe:Say(450,272,"CFOP",oFont08N:oFont)
oDanfe:Box(442,292,678,306)       //287
oDanfe:Say(450,295,"UN",oFont08N:oFont)
...
oDanfe:Box(442,340,678,385)
oDanfe:Say(450,350,"V.UNITARIO",oFont08N:oFont)
oDanfe:Say(450,398,"V.TOTAL",oFont08N:oFont)
//oDanfe:Box(442,400,678,465)
oDanfe:Box(442,430,678,500)
//oDanfe:Say(450,402,"BC.ICMS",oFont08N:oFont)
oDanfe:Say(450,435,"BC.ICMS",oFont08N:oFont)
//oDanfe:Box(442,465,678,515)
oDanfe:Box(442,475,678,525)
//oDanfe:Say(450,467,"V.ICMS",oFont08N:oFont)
oDanfe:Say(450,480,"V.ICMS",oFont08N:oFont)
...
/* RETIRADO POR ADILSON EM 10/02/2012 - NE ATACADO
	...
	If lflag
		If nL > nMaxItemP2
			...
			If lMv_Logod
				...
				oDanfe:SayBitmap(000+nLinhavers,000,cLogoD,095,043)
			Else
				//oDanfe:SayBitmap(002+nLinhavers,003,cLogo,085,085)
				oDanfe:SayBitmap(000+nLinhavers,000,cLogo,095,043)
			EndIf
			//-------
			...
			oDanfe:Box(163+nLinhavers,000,nColLim,543)//603
			oDanfe:Say(171+nLinhavers,002,"COD.",oFont08N:oFont)
			oDanfe:Box(163+nLinhavers,22,nColLim,219)
			oDanfe:Say(171+nLinhavers,028,"DESCRIÇÃO DO PROD./SERV.",oFont08N:oFont)  //030
			oDanfe:Say(171+nLinhavers,226,"NCM/SH",oFont08N:oFont)
			oDanfe:Box(163+nLinhavers,270,nColLim,254)   //230
			oDanfe:Say(171+nLinhavers,257,"CST",oFont08N:oFont)
			oDanfe:Say(171+nLinhavers,272,"CFOP",oFont08N:oFont)
			oDanfe:Box(163+nLinhavers,292,nColLim,306)       //287
			oDanfe:Say(171+nLinhavers,295,"UN",oFont08N:oFont)
			oDanfe:Say(171+nLinhavers,314,"QUANT.",oFont08N:oFont)
			oDanfe:Box(163+nLinhavers,340,nColLim,385)
			oDanfe:Say(171+nLinhavers,350,"V.UNITARIO",oFont08N:oFont)
			oDanfe:Say(171+nLinhavers,398,"V.TOTAL",oFont08N:oFont)
			oDanfe:Box(163+nLinhavers,430,nColLim,500)
			oDanfe:Say(171+nLinhavers,435,"BC.ICMS",oFont08N:oFont)
			oDanfe:Box(163+nLinhavers,475,nColLim,525)
			oDanfe:Say(171+nLinhavers,480,"V.ICMS",oFont08N:oFont)
...
/*
			//RETIRADO POR ADILSON EM 10/02/2012
			oDanfe:Say(161+nLinhavers,002,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont)
...
*/
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		If lMv_Logod
			...
			oDanfe:SayBitmap(000,000,cLogoD,095,043)
		Else
			//oDanfe:SayBitmap(000,000,cLogo,095,096)
			oDanfe:SayBitmap(000,000,cLogo,095,043)
		EndIf
		//-----------
...
*/	
	...
	oDanfe:Say(nLinha,028,aAux[1][2][nY],oFont08:oFont)// DESCRICAO DO PRODUTO  030
	oDanfe:Say(nLinha,220,aAux[1][3][nY],oFont08:oFont)//NCM     157
	oDanfe:Say(nLinha,255,aAux[1][4][nY],oFont08:oFont)//CST    192
	oDanfe:Say(nLinha,272,aAux[1][5][nY],oFont08:oFont)//CFOP   212
	oDanfe:Say(nLinha,294,aAux[1][6][nY],oFont08:oFont)//UN    232
	oDanfe:SayAlign(nLinha-6.5,293,aAux[1][7][nY],oFont08:oFont,45, 10, , 1,  )//QUANT       243
	oDanfe:SayAlign(nLinha-6.5,330,aAux[1][8][nY],oFont08:oFont,52, 10, , 1,  )//V UNITARIO  293
	oDanfe:SayAlign(nLinha-6.5,375,aAux[1][9][nY],oFont08:oFont,50, 10, , 1,  )//V. TOTAL   348 375
//	alert(aAux[1][9][nY])
//	oDanfe:Say(nLinha,395,aAux[1][9][nY],oFont08:oFont)//V. TOTAL   348 375
	oDanfe:SayAlign(nLinha-6.5,422,aAux[1][10][nY],oFont08:oFont,50, 10, , 1,  )//BC. ICMS   412
	oDanfe:SayAlign(nLinha-6.5,467,aAux[1][11][nY],oFont08:oFont,45, 10, , 1,  )//V. ICMS
	oDanfe:SayAlign(nLinha-6.5,504,aAux[1][12][nY],oFont08:oFont,45, 10, , 1,  )//V.IPI
	oDanfe:SayAlign(nLinha-6.5,546,aAux[1][13][nY],oFont08:oFont,30, 10, , 1,  )//A.ICMS
	oDanfe:SayAlign(nLinha-6.5,566,aAux[1][14][nY],oFont08:oFont,36, 10, , 1,  )//A.IPI
   //------------	
...
local cProg
cProg := iif(existBlock("LJDNFNFE"),"U_LJDNFNFE","LJDNFNFE")
&cProg.(oNFE, cCodAutSef, dDtReceb, cDtHrRecCab)
Return Nil
...
User Function TSPEDNFe()  // Carregar direto pelo smartclient
MsApp():New("SIGAFAT")
oApp:CreateEnv()
PtSetTheme("OCEAN")
oApp:cStartProg := "SPEDNFe"
__lInternet     := .T.
oApp:Activate()
Return Nil
