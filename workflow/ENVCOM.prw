...
User Function ENVCOM
	...
	cAccount	:= "erp2@grupoelizabeth.com.br" //GetMv("EL_RLCONTA")
	...
	cQuery += " A2.A2_EMAIL LIKE '%@%' AND A2.D_E_L_E_T_=' '  "
...
Static Function envia(cAccount, cPassword, cServer, cAssunto, cBody, cAnexo)
	...
	While (cAliasTmp)->(!EoF())
		...
		If nCount == 50
			U_ACSendM(cAccount,cPassword,cServer,"erp2@grupoelizabeth.com.br",cMail,cAssunto,cBody,cAnexo)
			cMail  := ""
			nCount := 0
		EndIf
        ...
        oProcess:IncReguA2("Enviando email "+cValToChar(i)+" de "+cValToChar(nQtd))
		cMail += (cAliasTmp)->EMAIL+"; "
		nCount++
		i++
		(cAliasTmp)->(DbSkip())
	End
    
	//U_ACSendM(cAccount,cPassword,cServer,"erp@grupoelizabeth.com.br","mateus.ramos@coderp.inf.br",cAssunto,cBody,cAnexo)
	//U_ACSendM(cAccount,cPassword,cServer,"erp@grupoelizabeth.com.br","aline.andrade@grupoelizabeth.com.br",cAssunto,cBody,cAnexo)
