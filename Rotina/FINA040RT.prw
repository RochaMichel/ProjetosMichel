...
/*/
//-------------------------------------------------------------------
Static Function LoadFK1(oGridModel,lCopia) 
	Local aBaixas  := {}
	Local aFK1Stru := FK1->(dbStruct())
	Local aCampos  := {}
	Local aAux     := {}
	Local cAlias   := CriaTrab(,.F.)
	Local cSelect  := ""
	Local cQry	   := ""
	Local cKeySE1  := xFilial("SE1",SE1->E1_FILORIG) + "|" +;
				SE1->E1_PREFIXO	+ "|" +;
				SE1->E1_NUM		+ "|" +;
				SE1->E1_PARCELA	+ "|" +;
				SE1->E1_TIPO	+ "|" +;
	...
	*/
	...
	While !(cAlias)->(EoF())
		...
		For nX := 1 to Len(aCampos)	// 		Formata data ( / / )
			aAdd( aAux, If( aCampos[nX][2] == "D", StoD((cAlias)->&(aCampos[nX][1])), (cAlias)->&(aCampos[nX][1]) ) )
		Next nX
