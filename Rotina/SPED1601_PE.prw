...
User Function SPED1601()
    ...
    */
    ...
    cQuery += "  AND F2_TIPO = 'N'   AND SF2.D_E_L_E_T_ = ' '  "
    cQuery += " GROUP BY F2_FILIAL, F2_CLIENTE, F2_LOJA , F2_CODA1U "
    ...
    While !(cAliasTrb)->( Eof() )
        ...
        //cCodAdm := PadR((cAliasTrb)->CLIENTE,TamSx3("A1_COD")[1])+(cAliasTrb)->LOJA // "C09   01" // CODIGO + LOJA -> AE_CODCLI + AE_LOJCLI
        cCodAdm := (cAliasTrb)->CLIENTE+IIF(Len((cAliasTrb)->CLIENTE)<6," ","")+(cAliasTrb)->LOJA // "C09   01" // CODIGO + LOJA -> AE_CODCLI + AE_LOJCLI
  
        aADD(aReg1601, {(cAliasTrb)->FILIAL,cCodAdm,"",(cAliasTrb)->TOT_VS,(cAliasTrb)->TOT_ISS,(cAliasTrb)->TOT_OUT})
