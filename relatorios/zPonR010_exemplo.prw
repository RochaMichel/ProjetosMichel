/**
	...
	Atilio, 11/11/2021
	Fonte foi congelado, através do PONR010 para ter o filtro de Supervisor e Região
	conforme lógica no SPGPER07

	Foi necessário o congelamento do arquivo, pois ele não possuía pontos de entrada
...
#Define STR0101 "Log de ocorrências na integração com TAE"
#Define STR0102 "Não foi possível efetuar autenticação no TAE"
#Define STR0103 "Verifique os parâmetros MV_SIGNURL, MV_RHTAEUS e MV_RHTAEPW"
#Define STR0104 "LIB desatualizada"
#Define STR0105 "Para execução da integração com o Totvs Assinatura Eletrônica é necessário que a LIB esteja atualizada com versão igual ou superior a 02/12/2021"
#Define STR0106 "Espelhos de Ponto enviados: "
#Define STR0107 "Espelhos de Ponto não enviados:"
#Define STR0108 "E-mail não cadastrado. Não foi enviado solicitação de assinatura para o colaborador."
#Define STR0109 "Solicitação enviada para "
#Define STR0110 'Acesse a opção "Config. Assina. Eletr." na rotina Controle de Espelho de Ponto para configurar o usuário e senha de integração com o Totvs Assinatura Eletrônica'
#Define STR0111 "Horários"
#Define STR0112 "Admissão: "
#Define STR0113 "Jornada"
#Define STR0114 "Não foi possível realizar o upload do arquivo para o TAE. Verifique se o usuário utilizado para a integração possui permissão para enviar arquivos."
#Define STR0115 "Marcações Desconsideradas"
#Define STR0116 "Hora da marcação"
#Define STR0117 "Motivo"
#Define STR0118 "Legenda das marcações: O: Original, I: incluída, P:Pré-assinalada"
...
/*
	 Variável usada para controlar os totalizadores status normais [1] e ausentes [2]
*/
...
//Atilio, 30/12/2021, conforme alinhado, foi alterado a database para ser a data até
//   para que o relatório não saia vazio (pdf em branco)
dDataBase := dPerFim
	...
	Static cPathTmp	 := GetTempPath(.F.)
			...
			//alert("continuando relatorio")
				...
				If !lTAE .and. !lTerminal .And. !lMeuRH
					...
					//ja entra sem precisar do box das validaçoes

				ElseIf lMeuRH .Or. lTAE
					aMargRel := {10,10,10,10}
					cFile    := cFileName
					cLocal	 := SuperGetMV("MV_RELT", .F., "\spool\") + "espelho_ponto\" // 21/04/2022, mudando a pasta default para gerar o relatório
					If lTAE
...
//Local lOk			:= .F.
Local lPrinter		:= .F.
//- Utilizadas apenas para validação do looping
Local nAtual 		:= 0
Local nTotal 		:= 0
...
//Monta o Where customizado com o filtro de supervisor e região
cWhereCust := "%1 = 1%"
...
While (cAliasSRA)->( !Eof() )
	...
	//SRA->(DbSeek((cAliasSRA)->RA_FILIAL + (cAliasSRA)->RA_MAT)) //-- 2023-07-18, Linha comentada pois ao invés de posicionar estava desposicionando
...
If !( lTerminal )
	...
	/*
		Atilio, 30/12/2021, mudanças no layout, sendo:
		1. A Função ficará no lugar da Chapa
		2. Na onde estava a Função ficará a Região
		3. Na segunda coluna e linha do Situação, ficará o Gerente
		4. Na terceira coluna e linha da Situação, ficará o Supervisor
	*/
...
Static Function SendEsp(cPathFile, cNameFile)
	...
	Else
		cMsg := If(jResponse[ "description" ] != NIL, jResponse[ "description" ], STR0114) //"Não foi possível realizar o upload do arquivo para o TAE. Verifique se o usuário utilizado para a integração possui permissão para enviar arquivos."
