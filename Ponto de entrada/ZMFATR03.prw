...
Static Function Impress(oPrint, cAlias)
	...
	While !oPrint:Cancel() .And. !(cAlias)->(Eof())
		...
		If nCount <> nRecCount .And. nRecCount <> 1
			If  len(AllTrim(SC6->C6_DESCRI)+" - ZUMMI ") > 45
				oPrint:Box(nLinBox-10,0015, nLinBox+12, 580, "-4")
			...
			Else
				...
				oPrint:Line(nLin-25,0035 ,nLinBox+10, 0035)//fim item
				oPrint:Line(nLin-25,0067 ,nLinBox+10, 0067)//fim codigo
				oPrint:Line(nLin-25,0235 ,nLinBox+10, 0235)//fim descri
				oPrint:Line(nLin-25,0255 ,nLinBox+10, 0255)//fim da um
				oPrint:Line(nLin-25,0300 ,nLinBox+10, 0300)//fim ncm
				oPrint:Line(nLin-25,0345 ,nLinBox+10, 0345)//fim quant
				oPrint:Line(nLin-25,0395 ,nLinBox+10, 0395)//fim preço
				oPrint:Line(nLin-25,0430 ,nLinBox+10, 0430)//fim icms
				oPrint:Line(nLin-25,0465 ,nLinBox+10, 0465)//fim vlr st
				oPrint:Line(nLin-25,0500 ,nLinBox+10, 0500)//fim ipi
				oPrint:Line(nLin-25,0535 ,nLinBox+10, 0535)//fim frete
				oPrint:Line(nLin-25,0580 ,nLinBox+10, 0580)//fim total
			EndIF
		Endif
...
Static Function fCabec(oPrint, cAlias)
  ...
  //Carrega vetor com dados da empresa
    ...
    nLin+=nSpace30
    oPrint:Box(nLin,0015, nLin+50, 580, "-4")
    oPrint:Box(nLin,0015, nLin+50, 290, "-4")
